...
src = cv2.imread("4_sudoku.png", cv2.IMREAD_GRAYSCALE)
...
# Perform dilation to connect digits of each number
...
# Display the dilated image and the image with bounding boxes
cv2.imshow('Dilated Image', dilated_image)
cv2.imshow('Image with Bounding Boxes', image_copy)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
_, binary_image = cv2.threshold(img_gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
#Apply opening and closing to remove noise and connect broken digits
kernel = np.ones((3, 3), np.uint8)
opened_image = cv2.morphologyEx(binary_image, cv2.MORPH_OPEN, kernel)
closed_image = cv2.morphologyEx(opened_image, cv2.MORPH_CLOSE, kernel)
# Find contours and bounding boxes
contours, _ = cv2.findContours(closed_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
# Draw rectangles surrounding each digit
for cnt in contours:
    x, y, w, h = cv2.boundingRect(cnt)
